...
# Initialize our solution. This change vector is going to represent the number of ways to make the target value with only the coins allowed so far.
change = [0 for x in range(0, 201)]
change[0] = 1 #There's one way to make no money with no coins.

for coin in coins: #We'll start by building the answer to the question "How many ways can you make change with a penny", then adding in coins one by one.
